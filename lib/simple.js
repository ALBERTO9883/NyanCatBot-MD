...
END:VCARD
    ...
    return conn.relayMessage(jid, prep.message, { messageId: prep.key.id })
        ...
        let prep = generateWAMessageFromContent(jid, { liveLocationMessage: { degreesLatitude: 22.685506256233525, degreesLongitude: 124.75250667105852, caption: text, sequenceNumber: 1656662952682001, timeOffset: 8600, jpegThumbnail: null }}, { quoted: quoted })
...
return conn.relayMessage(jid, prep.message, { messageId: prep.key.id })
    ...
    requestPaymentMessage: {
      ...
      sendShoppingCart: {
      async value(jid, text = '', buffer, quoted, options) {
      if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) } catch { buffer = buffer } 
      const prep = generateWAMessageFromContent(jid, { "orderMessage": { "orderId":"6288215463787", "itemCount": 2023, "message": text, "orderTitle": global.botname, "footerText": "NyanCatBot - MD", "token": "AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA==", "thumbnail": buffer, "surface": "CATALOG", contextInfo: { mentionedJid: conn.parseMention(text) } }}, { quoted: quoted })
await conn.relayMessage(jid, prep.message,  { messageId: prep.key.id })
}
},      
         sendProductArticle: {
async value(jid, text = '', buffer, amount, url, quoted, options) {
    if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) } catch { buffer = buffer }

const messa = await prepareWAMessageMedia({ image: buffer }, { upload: conn.waUploadToServer })
	const prep = generateWAMessageFromContent(jid, proto.Message.fromObject({
		"productMessage": {
		"product": {
			...
			"productImageCount": 1,
			"firstImageId": 1,
			"salePriceAmount1000": amount,
			"retailerId": text,
			"url": url
		},
			"businessOwnerJid": "50499698072@s.whatsapp.net",
		}
...
return conn.relayMessage(jid, prep.message, { messageId: prep.key.id })
        ...
        sendLocUrl: {
            async value(jid, text = '', body, url, buffer, quoted, options) {
                if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) } catch { buffer = buffer }
            let msg = await generateWAMessageFromContent(jid, { locationMessage: {
                degreesLatitude: 0,
                degreesLongitude: 0,
                name: text,
                address: body,
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        ftext: {
        value(text) {
let prep = generateWAMessageFromContent(this.chat, { extendedTextMessage: {
   text: text,
   jpegThumbnail: ftextjpg, contextInfo: { mentionedJid: this.conn.parseMention(text) }}}, { quoted: this })
return this.conn.relayMessage(this.chat, prep.message, { messageId: prep.key.id })
}},
        copy: {
            value() {
                const M = proto.WebMessageInfo
