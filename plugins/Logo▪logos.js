...
if (!effect) throw '*◈ ¿𝗖𝗢́𝗠𝗢 𝗦𝗘 𝗨𝗧𝗜𝗟𝗜𝗭𝗔 𝗘𝗦𝗧𝗘 𝗖𝗢𝗠𝗔𝗡𝗗𝗢?*\n➧ _#logo (efecto) (texto)_\n*Eᴊᴇᴍᴘʟᴏ:*\n➧ _#logo 3d-deep-sea-metal Mystic_\n\n*◈ 𝗖𝗨𝗔𝗡𝗗𝗢 𝗜𝗡𝗗𝗜𝗤𝗨𝗘 𝗤𝗨𝗘 𝗙𝗔𝗟𝗧𝗔 𝗨𝗡 𝗧𝗘𝗫𝗧𝗢,𝗦𝗘 𝗨𝗦𝗔𝗥𝗜́𝗔 𝗔𝗦𝗜́:*\n➧ _#logo (efecto) (texto1|texto2)_\n*Eᴊᴇᴍᴘʟᴏ:*\n➧ _#logo Wolf-Logo-Galaxy Mystic|Bot_\n\n\t\t*◈「 _•🪴Lɪsᴛᴀ Dᴇ Eғᴇᴄᴛᴏs🪴•_ 」◈*\n━━━━━━━━━━━━━━━━━━━\n*┃›📝* #logo ' + effects.map(v => v.title).join('\n*┃›📝* #logo ')
effect = effect.toLowerCase()
if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `*❎Eʟ ᴇғᴇᴄᴛᴏ ${effect} Nᴏ ᴇsᴛᴀ́ ᴇɴ ʟᴀ ʟɪsᴛᴀ ᴅᴇ ᴇғᴇᴄᴛᴏs*`
let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
if (text.includes(split)) text = text.split(split)
text = Array.isArray(text) ? text : [text]
let res = await textpro(effect, ...text)
if (typeof res == 'number') throw res == -1 ? `*❎Eʟ ᴇғᴇᴄᴛᴏ ${effect} Nᴏ ᴇsᴛᴀ́ ᴇɴ ʟᴀ ʟɪsᴛᴀ ᴅᴇ ᴇғᴇᴄᴛᴏs*` : `*[❗𝐈𝐍𝐅𝐎❗] 𝙴𝙻 𝚄𝚂𝙾 𝙲𝙾𝚁𝚁𝙴𝙲𝚃𝙾 𝙳𝙴𝙻 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝙴𝚂 ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
